...
const SearchResultItem = ({data}) => {
  ...
  const handleChooseTicket = () => {
    navigate(`/admin/ticket/${data.id}`);
  }

  return (
    <div onClick={handleChooseTicket}  className="flex items-center mb-[1rem] hover:bg-[#ebecf0] px-[0.5rem] py-[0.3rem] rounded-sm cursor-pointer">
      ...
      <div className="w-[50%] text-[1rem] mr-[2rem] text-[#172B4D]">{`${data.title}`}</div>
